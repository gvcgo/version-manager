...
- **跨平台**的Asciinema**终端录屏**功能，支持asiinema文件的后期处理，上传到官网等；同时，配合vm安装的agg工具，还能一键转换为gif图片，可谓写文档的利器；
- CLOC(Count Lines Of Code)代码统计功能，支持各种语言，可以一键统计项目或者文件夹下的各种代码的行数、注释行数、空行数等等，项目情况了然于胸；
...
- gopher增强工具：1、**一键跨平台编译并压缩打包**，无需写脚本；2、一键重命名本地go package，无需搜索替换； 3、一键安装一些知名的go项目，例如go-zero的goctl，goframe的gf，跨平台Desktop应用开发工具wails，go-grpc相关的工具，几种用go编写的压力测试工具，gopls、dlv等等，你无须自己再去找这些，一键选择安装即可；
