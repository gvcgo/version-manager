...
在这里厚颜推荐一下gvcgo下面的另一个项目[gvc](https://github.com/gvcgo/gvc)。其实，**gvc**之前就是做跨平台SDK版本管理的。只是它当时集成了很多其他功能，很多人不能理解，另外还包含了免费梯子，所以一直无法推广。**gvc**做得要比vfox早很多。现在的**vm**就是从**gvc**中剥离并改进而来的。把一些踩过的坑进行了从新设计。尽量保证稳定性和可用性，摒弃一些会带来不稳定因素的东西，拒绝花里胡哨，主打实用。

经过剥离之后，[gvc](https://github.com/gvcgo/gvc)已经不含梯子和版本管理，但仍然有着一些对程序员来说比较好用和方便的功能。主要有以下几个方面：
- **跨平台**的Asciinema**终端录屏**功能，支持asciinema文件的后期处理，上传到官网等；同时，配合vm安装的agg工具，还能一键转换为gif图片，可谓写文档的利器；
- CLOC(Count Lines Of Code)**代码统计**功能，支持各种语言，可以一键统计项目或者文件夹下的各种代码的行数、注释行数、空行数等等，项目情况了然于胸；
- git相关的杂项：1、**一键更新hosts文件**，能加速github、vscode官网、steam等网站的访问； 2、一键设置git的ssh代理，**让你的本地代理可以给到ssh协议使用**，主要是加速github项目的命令行操作；可以一键在有无代理之间切换；
- gopher增强工具：1、**一键跨平台编译并压缩打包**，无需写脚本；2、一键重命名本地go package，无需搜索替换； 3、一键安装一些**知名的go项目**，例如**go-zero**的goctl，**goframe**的gf，跨平台Desktop应用开发工具**wails**，**go-grpc**相关的工具，几种用go编写的**压力测试**工具，gopls、dlv等等，你无须自己再去找这些，**一键选择安装**即可；
- 基于TUI的GPT终端机器人：支持**ChatGPT**和**讯飞星火**；
- 基于Github/Gitee仓库的**备份**和**图床**功能：1、一键将 vscode、asiinema、gpt、.ssh等配置文件加密后上传到自己设置的远程仓库，例如上传加密压缩后的.ssh文件夹之后，你就可以在任何机器上通过**gvc**进行**一键恢复**.ssh文件夹，这样就无需在新机器上重新生成key，也能访问github或者服务器了；2、可以设置一个远程仓库作为图床，一键上传本地图片到这个仓库，然后会回显图片地址(对于github，还会生成国内加速的地址)，**可以在markdown中直接使用**；
