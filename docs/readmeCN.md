...
- 基于Github/Gitee仓库的备份和图床功能：1、一键将 vscode、asiinema、gpt、.ssh等配置文件加密后上传到自己设置的远程仓库，例如上传加密压缩后的.ssh文件夹之后，你就可以在任何机器上通过**gvc**进行一键恢复.ssh文件夹，这样就无需在新机器上重新生成key，也能访问github或者服务器了；2、可以设置一个远程仓库作为图床，一键上传本地图片到这个仓库，然后会回显图片地址(对于github，还会生成国内加速的地址)，可以在markdown中直接使用；
